...
def pivot_months_pandas(data):
    ...
    month_co = data.groupby(['name','month'])['precipitation'].aggregate('sum').reset_index()
    count_co = data.groupby(['name','month'])['precipitation'].aggregate('count').reset_index()
    
    monthly = month_co.pivot(index='name', columns='month', values='precipitation')
    counts = count_co.pivot(index='name', columns='month', values='precipitation')
